...
const RULES = {
  ...
  topicRoomsPerWeek: 3,
  challengesPerWeek: 7,
  challengeDoneWhenMin: 12,
  challengeDoneWhenMax: 160,
  challengeDaysMin: 1,
  challengeDaysMax: 7,
};
...
const RESERVED_SLUGS = new Set([
  ...
  "rooms", "room", "topic", "topics", "meetup", "static", "css", "js", "pair",
]);
...
function normalizeBoard(raw) {
  ...
  for (const card of next.cards) {
    ...
    if (typeof card.doneWhen !== "string") card.doneWhen = card.doneWhen || "";
    if (typeof card.whoCanTry !== "string") card.whoCanTry = card.whoCanTry || "";
    if (!card.challengeUntil) card.challengeUntil = 0;
    if (!card.challengeOpenedAt) card.challengeOpenedAt = 0;
  }
...
function wantsChallenge(input) {
  const doneWhen = String((input && input.doneWhen) || "").trim();
  const flag = input && (input.challenge === true || String(input.challenge || "").toLowerCase() === "on");
  return Boolean(doneWhen || flag);
}

function parseChallenge(input, now) {
  if (!wantsChallenge(input)) return { ok: true, challenge: null };
  const doneWhen = String((input && input.doneWhen) || "").trim();
  if (doneWhen.length < RULES.challengeDoneWhenMin || doneWhen.length > RULES.challengeDoneWhenMax) {
    return { ok: false, error: "Done when needs one real sentence a stranger can check." };
  }
  if (leakHits(doneWhen)) {
    return { ok: false, error: "That done-when looks like it has private stuff in it. Take out paths, keys, and emails." };
  }
  const days = Math.floor(Number((input && (input.challengeDays ?? input.days)) ?? 7));
  if (!Number.isFinite(days) || days < RULES.challengeDaysMin || days > RULES.challengeDaysMax) {
    return { ok: false, error: "Give the challenge a clock of one to seven days." };
  }
  const whoRaw = String((input && input.whoCanTry) || "anyone").trim().toLowerCase();
  const whoCanTry = whoRaw === "this-room" ? "this-room" : "anyone";
  return {
    ok: true,
    challenge: {
      doneWhen,
      whoCanTry,
      challengeUntil: now + days * DAY_MS,
      challengeOpenedAt: now,
    },
  };
}

function applyChallengeToCard(card, challenge) {
  if (!card || !challenge) return;
  card.doneWhen = challenge.doneWhen;
  card.whoCanTry = challenge.whoCanTry;
  card.challengeUntil = challenge.challengeUntil;
  card.challengeOpenedAt = challenge.challengeOpenedAt;
}

function challengeView(card, now) {
  const doneWhen = String((card && card.doneWhen) || "").trim();
  const until = Number((card && card.challengeUntil) || 0);
  if (!doneWhen || !until) return null;
  return {
    doneWhen,
    whoCanTry: String(card.whoCanTry || "") === "this-room" ? "this-room" : "anyone",
    until,
    open: until > now,
  };
}

function ownsProblem(card, input) {
  if (!card) return false;
  const userId = inputUserId(input) || String((input && input.authorId) || "").trim();
  if (userId && card.authorId && userId === card.authorId) return true;
  if (!card.authorId) {
    const name = String((input && input.authorName) || "").trim().toLowerCase();
    if (name && name === String(card.authorName || "").trim().toLowerCase()) return true;
  }
  return false;
}

function challengesThisWeek(board, input, now) {
  const key = weekKey(now);
  const userId = inputUserId(input) || String((input && input.authorId) || "").trim();
  const name = String((input && input.authorName) || "").trim().toLowerCase();
  return board.cards.filter((c) => {
    if (isAnswerCard(c)) return false;
    if (!c.challengeOpenedAt || weekKey(c.challengeOpenedAt) !== key) return false;
    if (userId && c.authorId && c.authorId === userId) return true;
    if (!c.authorId) {
      const cardName = String(c.authorName || "").trim().toLowerCase();
      if (name && cardName && name === cardName) return true;
    }
    return false;
  }).length;
}
...
function addCard(board, input, now) {
  ...
  const parsed = parseChallenge(input, now);
  if (!parsed.ok) return parsed;
  if (parsed.challenge) {
    const owner = {
      ...input,
      authorName: checked.card.authorName,
      authorId: checked.card.authorId,
    };
    if (challengesThisWeek(board, owner, now) >= RULES.challengesPerWeek) {
      return { ok: false, error: "That's this week's challenges. Come back next week." };
    }
    applyChallengeToCard(checked.card, parsed.challenge);
  }
...
function openChallenge(board, input, now) {
  const channel = findChannel(board, input.channelId);
  if (!canSeeFromInput(channel, input)) {
    return { ok: false, error: "That room is closed. You need the link." };
  }
  if (!inputUserId(input)) {
  ...
  if (!ownsProblem(card, input)) {
    return { ok: false, error: "Only the person who posted it can open a challenge." };
  }
  const current = challengeView(card, now);
  if (current && current.open) {
    return { ok: false, error: "This is already an open challenge." };
  }
  const parsed = parseChallenge({
    ...input,
    challenge: true,
    doneWhen: input.doneWhen,
    challengeDays: input.challengeDays ?? input.days ?? 7,
    whoCanTry: input.whoCanTry,
  }, now);
  if (!parsed.ok) return parsed;
  if (!parsed.challenge) {
    return { ok: false, error: "Done when needs one real sentence a stranger can check." };
  }
  if (challengesThisWeek(board, input, now) >= RULES.challengesPerWeek) {
    return { ok: false, error: "That's this week's challenges. Come back next week." };
  }
  applyChallengeToCard(card, parsed.challenge);
  return { ok: true, card, challenge: challengeView(card, now) };
}
...
function publicTopicsPermalink(origin) {
  const base = String(origin || "").replace(/\/$/, "");
  return `${base}/topics`;
}

function publicMeetupPermalink(origin) {
  const base = String(origin || "").replace(/\/$/, "");
  return `${base}/meetup`;
}

function publicMeetupView(board, now) {
  const when = Number(now) || Date.now();
  const cards = channelCards(board, "public");
  const votes = channelVotes(board, "public");
  const top = ranked(cards, votes, when - WEEK_MS)
    .slice(0, 10)
    .map((row) => ({
      id: row.card.id,
  ...
  return {
    winner: first
      ? { id: first.id, title: first.title, authorName: first.authorName, rank: first.rank }
      : null,
    top,
  };
}

function publicTopicRooms(board) {
  return (board.channels || [])
    .filter((c) => c && c.visibility === "public" && c.kind === "topic")
    .slice()
...
function publicSystemView(board, card, now) {
  ...
  return {
    ...
    children,
    answers,
    challenge: challengeView(card, when),
  };
}
...
function publicSitemapXml(board, origin) {
  const base = String(origin || "").replace(/\/$/, "");
  const locs = [base + "/", publicTopicsPermalink(base), publicMeetupPermalink(base)];
...
function snapshot(board, channelId, joinKey, now, viewer) {
  ...
  const decorate = (row) => {
    ...
    return {
      ...
      parent: parent ? { id: parent.id, title: parent.title } : null,
      children,
      challenge: challengeView(row.card, now),
  ...
  const challenges = rankedAll.filter((row) => row.challenge && row.challenge.open);
  const top = {
...
module.exports = {
  ...
  publicPermalink,
  publicRoomPermalink,
  publicTopicsPermalink,
  publicMeetupPermalink,
  publicMeetupView,
  publicTopicRooms,
  publicSystemView,
  publicSitemapXml,
