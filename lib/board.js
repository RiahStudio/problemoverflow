...
function channelCards(board, channelId) {
  return board.cards.filter((c) => c.channel === channelId && c.kind !== "reply");
}
...
function addVote(board, input, now) {
  ...
  if (!findProblem(board, channel.id, cardId)) {
...
function setBuzzLink(board, input) {
  ...
  if (!/^https:\/\/(www\.)?buzz\.xyz(\/|$)/i.test(url)) {
    return { ok: false, error: "That has to be a Buzz link." };
  }
  if (leakHits(url) || /127\.0\.0\.1|localhost/i.test(url)) {
    return { ok: false, error: "That link is not safe." };
  }
  card.buzzUrl = url;
  return { ok: true, card };
}
...
function findTallyCard(board, item) {
  ...
  if (cardId) {
    const byId = board.cards.find((c) => c.id === cardId && c.kind !== "reply");
    if (byId) return byId;
  }
  const url = normalizeBuzzUrl(item && item.buzzUrl);
  if (!url) return null;
  return board.cards.find((c) => c.kind !== "reply" && normalizeBuzzUrl(c.buzzUrl) === url) || null;
}
...
function snapshot(board, channelId, joinKey, now, viewer) {
  ...
  const decorate = (row) => ({
    ...row,
    card: stripForViewer(row.card, view.role),
    replies: replies.filter((r) => r.inReplyTo === row.card.id).sort((a, b) => a.createdAt - b.createdAt),
  });
