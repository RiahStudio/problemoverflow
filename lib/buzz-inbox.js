...
function inboxDir(root, dataDir) {
  return path.join(dataDir || path.join(root, "data"), "buzz-inbox");
}

function foldFromDisk(root, now, loadBoard, saveBoard, dataDir) {
  const dir = inboxDir(root, dataDir);
...
function startHourly(root, loadBoard, saveBoard, dataDir) {
  const run = () => {
    try {
      foldFromDisk(root, Date.now(), loadBoard, saveBoard, dataDir);
    } catch {
